...
public class ConfigClientFallback implements ZuulFallbackProvider {
    private static final Logger LOGGER = LoggerFactory.getLogger(ConfigClientFallback.class);
    @Override
    public String getRoute() {
        return "attendance-config-client";
    }
    ...
    public ClientHttpResponse fallbackResponse() {
        return new ClientHttpResponse() {
            ...
            public InputStream getBody() throws IOException {
                LOGGER.error("zuul fallback " + ConfigClientFallback.this.getRoute().toString());
                ResultObj resultObj = ResultObj.errorWithLog(HttpStatus.INTERNAL_SERVER_ERROR.value(),
                        "服务请求失败",ConfigClientFallback.this.getRoute().toString());
